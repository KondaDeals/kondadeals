...
export async function fetchHomepageData() {
  ...
  const [bannersRes, catsRes, featuredRes, trendingRes, newRes] = await Promise.all([
    supabase
      .from('hero_banners')
    ...
    supabase
      .from('products')
      .select('id,name,slug,mrp,sale_price,images,is_trending,discount_ends_at,categories(name)')
      .eq('is_active', true)
      .eq('is_featured', true)
      .order('created_at', { ascending: false })
      .limit(8),
